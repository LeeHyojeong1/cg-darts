...
def main():
  ...
  if args.debug_fake_data:
    ...
    valid_data = dset.FakeData(
      size=max(args.batch_size, args.debug_num_samples // 2), image_size=(3, 32, 32),
      num_classes=CIFAR_CLASSES, transform=valid_transform)
  else:
    train_data = dset.CIFAR10(root=args.data, train=True, download=args.download, transform=train_transform)
    valid_data = dset.CIFAR10(root=args.data, train=False, download=args.download, transform=valid_transform)
